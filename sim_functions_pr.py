...
def sim_space_neurons_3D_dynamicPr(width = 100, depth = 10, dx_dy = 1, time = 1, D = 763, 
              inter_var_distance = 2.92, p_r = [0.06,0.06], f_rate = 4, n_neurons = 150, Hz = 0.01):
    ...
    p_r_series = np.interp(np.linspace(0,int(t/dt)-1,int(t/dt)), np.linspace(0,int(t/dt)-1,len(p_r)), p_r)
    ...
    for i in range(firing.shape[1]):
        terminal_refraction = np.random.choice(firing.shape[0],int(firing.shape[0]*(1-p_r_series[i])),replace = False)
        firing[terminal_refraction,i] = 0
...
def sim_dynamics_3D_dynamicKm(space0, space_ph, release_sites, firing, var_list, 
                 Q = 3000, uptake_rate = 4*10**-6, Km = [210*10**-9,210*10**-9],
                 Ds = 321.7237308146399, ECF = 0.21, rev_trans = [0,0], start_occ_D2 = 0.6):
    # print(uptake_rate)
    # print(Q)
    # Extract parameters
    t = var_list[0]
    ...
    dx_dy = var_list[2]
    Hz = var_list[4]
    Kapp = np.interp(np.linspace(0,int(t/dt)-1,int(t/dt)), np.linspace(0,int(t/dt)-1,len(Km)), Km)
    rev_trans = np.interp(np.linspace(0,int(t/dt)-1,int(t/dt)), np.linspace(0,int(t/dt)-1,len(rev_trans)), rev_trans)

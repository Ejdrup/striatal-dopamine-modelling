...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug 22 12:35:24 2022
...
simulation, space_ph, firing, release_sites, var_list = \
        sim_space_neurons_3D(width = 20, depth = 20, dx_dy = 0.2, time = 4, D = 763,
...
dist_DA_fine = np.percentile(full_sim_fine[100:,:,:,:],np.linspace(0,100,1001))
dist_DA_coarse = np.percentile(full_sim_coarse[100:,:,:,:],np.linspace(0,100,1001))

hist_DA_fine, _ = np.histogram(np.log10(full_sim_fine[100:,:,:,:]).flatten(), bins = np.log10(np.logspace(-10,-5,1000)), density = True)
hist_DA_coarse, _ = np.histogram(np.log10(full_sim_coarse[100:,:,:,:]).flatten(), bins = np.log10(np.logspace(-10,-5,1000)), density = True)
...
ax1.plot(x_range,dist_DA_fine*10**9, "palevioletred")
ax1.plot(x_range,dist_DA_coarse*10**9, "darkkhaki")
ax1.plot(x_range,abs(dist_DA_fine-dist_DA_coarse)*10**9, color = "dimgrey")
...
ax1.plot(x_range,dist_DA_large*10**9, "seagreen")
# ax1.legend(("0.2 \u00B5m","1.0 \u00B5m","Difference"), 
#            frameon = False, loc = "lower center", ncol = 3, handlelength = 1.2, handletextpad = 0.5,
#            columnspacing = 0.5,fontsize = 8)
